...
<body>
  ...
  $p = $_GET["p"];
	...
	$cha3="SELECT * FROM i where type='人物'";
	$cha_result3=mysqli_query($con,$cha3);
	$allpic=mysqli_num_rows($cha_result3);
	$allpage=	ceil($allpic/18);

	$people = 18*$p-18;
	$sql = "SELECT * FROM i where type='人物' ORDER BY id DESC limit $people,18";
    ...
    <div class="container">
			...
			<div class="page">
				...
				$page_rest = $allpage - $p;
				if($p > 2){
					echo '<a href="./?type=people&p='.$page_2.'">'.$page_2.'</a>'."\n";
				}
				if($p > 1){
					echo '<a href="./?type=people&p='.$page_1.'">'.$page_1.'</a>'."\n";
				}
				?>
				<a class="active nowpage"><?php echo $p; ?></a>
				<?php
				if($page_rest > 0){
					echo '<a href="./?type=people&p='.$page_3.'">'.$page_3.'</a>'."\n";
				}
			  if($page_rest > 1){
					echo '<a href="./?type=people&p='.$page_4.'">'.$page_4.'</a>'."\n";
				}
				?>
				<a href="./?type=people&p=<?php
				if($p == $allpage){
    ...
    <?php include "./php/footer.php";?>
		...
		$(".nowpage").click(function() {
			swal({
				title: "",
				text: "<div class='allpages'><?php $allpages=1;$allpage1=$allpage+1;while($allpages < $allpage1){echo "<a href='./?type=people&p=".$allpages."'>".$allpages."</a>";$allpages++;} ?></div>",
				html: true,
				type: null,
